...
const EditEmployee = () => {
  ...
  const {
    ...
    console.log("EditEmployee Redux state:", state.company);
    return state.company || {};
  });

  useEffect(() => {
    console.log("EditEmployee mounting, resetting state");
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      toast.error("Please fill in all fields", {
        toastId: "edit-employee-error",
      });
      return;
    }
    setHasSubmitted(true);
    console.log(
      "Submitting updated employee data:",
      formData,
      "for employeeId:",
      employeeId
    );
    try {
      await dispatch(
        updateEmployeeData({ employeeId, employeeData: formData })
      ).unwrap();
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (employeeError && hasSubmitted) {
      toast.error(employeeMessage, { toastId: "edit-employee-error" });
      dispatch(companyReset());
      setHasSubmitted(false);
    }
    if (employeeSuccess && hasSubmitted) {
      toast.success("Employee updated successfully!", {
        toastId: `edit-employee-success-${employeeId}`,
      });
      dispatch(companyReset());
      setFormData({
        name: "",
